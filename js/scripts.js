...
var obj ={
  ...
  window.onload=alltodos(); placeholder(); describe(); 
    ...
    if( document.getElementById('ul-1').hasChildNodes()===false){
      document.getElementById('placeholder').innerHTML="<p>There Is No Item In The List Yet.</p>"    
    }  
  }
  function describe(){
    if( document.getElementById('ul-detail').hasChildNodes()===false){
      document.getElementById('describe').innerHTML="<p> Detail List Will appear Here</p>"
    }
  } 
  function alltodos() {
  ...
  span.innerHTML="<i class='fas fa-pen prefix black-text' title='Edit' onclick='var q= this.parentNode.parentNode.innerText ; editing(q)' ></i><i class='fas fa-trash black-text' title='Delete' onclick='remove(i)'></i><i class='fas fa-info prefix black-text' onclick='var q= this.parentNode.parentNode.innerText ; detail(q)' title='Detail' ></i>"
  li.appendChild(span);
